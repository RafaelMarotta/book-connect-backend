...
exports.getTrocaById = async (req, res) => {
    ...
    const { id } = req.params;
    try {
        const [rows] = await db.query('SELECT t.id, t.data, l1.titulo as livro_oferecido, l2.titulo as livro_doado FROM troca t JOIN livro l1 ON l1.id = t.livro_oferecido_id JOIN livro l2 ON l2.id = t.livro_doado_id WHERE t.id = ?', [id]);
        if (rows.length === 0) {
